    ...
    switch (e->kind) {
        case T_BINOP: {
            ...
            if (left->kind == T_CONST) { /* BINOP(op, e1, CONST(i)) */
                sprintf(asm_str, "%s $0x%x, `d0", op2asm(e->u.BINOP.op), left->u.CONST);
                ...
                sprintf(asm_str, "%s $0x%x, `d0", op2asm(e->u.BINOP.op), right->u.CONST);
                emit(AS_Oper(String(asm_str), Temp_TempList(r, NULL),
                                          Temp_TempList(munchExp(left), NULL),
                                          NULL));
            } else { /* BINOP(op, e1, e2) */
                sprintf(asm_str, "%s `s0, `d0", op2asm(e->u.BINOP.op));
                emit(AS_Oper(String(asm_str), Temp_TempList(r, NULL),
        ...
        case T_MEM: {
            ...
            if (mem->kind == T_BINOP && mem->u.BINOP.op == T_plus) { /* MEM(BINOP(+, CONST, e) */
                ...
                sprintf(asm_str, "mov 0x%x(`s0), `d0", ct->u.CONST);
                emit(AS_Oper(String(asm_str), Temp_TempList(r, NULL),
                                          Temp_TempList(munchExp(ct), NULL),
                                          NULL));
            } else if (mem->kind == T_CONST) {
                sprintf(asm_str, "mov (0x%x), `d0", mem->u.CONST);
                emit(AS_Oper(String(asm_str), Temp_TempList(r, NULL), NULL, NULL));
            } else {
                emit(AS_Oper(String("mov (`s0), `d0"), Temp_TempList(r, NULL),
        ...
        case T_CONST: {
            sprintf(asm_str, "mov $0x%x, `d0", e->u.CONST);
            emit(AS_Oper(String(asm_str), Temp_TempList(r, NULL), NULL, NULL));
            return r;
        }
        case T_CALL: {
            ...
            emit(AS_Oper(String("call `s0"), F_calldefs(), Temp_TempList(r, l), NULL));
            return r;
        }
    }
}
            ...
            if (dst->kind == T_MEM) {
                if (dst->u.MEM->kind == T_BINOP && dst->u.MEM->u.BINOP.op == T_plus) {
                    ...
                    if (dst->u.MEM->u.BINOP.right->kind == T_CONST) {
                        sprintf(asm_str, "mov `s1, 0x%x(`s0)", dst->u.MEM->u.BINOP.right->u.CONST);
                    ...
                    if (dst->u.MEM->u.BINOP.left->kind == T_CONST) {
                        sprintf(asm_str, "mov `s1, 0x%x(`s0)", dst->u.MEM->u.BINOP.left->u.CONST);
                        emit(AS_Oper(String(asm_str), NULL,
                                    Temp_TempList(munchExp(dst->u.MEM->u.BINOP.right),
                                    Temp_TempList(munchExp(src), NULL)), NULL));
                    }
                } else if (src->kind == T_MEM) { /* MOVE(MEM(e1), MEM(e2)) */
                    emit(AS_Oper(String("mov `s1, (`s0)"), NULL,
                    ...
                    sprintf(asm_str, "mov `s0, (0x%x)", dst->u.MEM->u.CONST);
